...
from core.telemetry import get_request_id
...
async def chat(
    ...
    try:
        completion = await complete_invoice_draft(
            draft,
            idempotency_key=f"chat:{draft.invoice_number or uuid4().hex}",
        )
    ...
    if missing_fields:
        response = AiChatMissingFieldsResponse(
            ...
            missing_fields=missing_fields,
            draft=draft,
            fields_to_show=completion.analysis.fields_to_show,
        )
        log_event(
            "ai.chat.response.sent",
            level=logging.WARNING,
    ...
    log_event(
        ...
        created_invoice=summarize_created_invoice(
            {
                "id": created_invoice.document_id,
                "invoice_number": created_invoice.invoice_number,
                "subtotal": created_invoice.subtotal,
                "total": created_invoice.total,
                "currency": created_invoice.currency,
