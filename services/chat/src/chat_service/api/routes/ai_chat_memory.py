...
from chat_service.clients.document import DocumentConflictError, DocumentUnavailableError
from chat_service.core.config import LLAMA_MODEL_FILE, LLM_CHAT_MAX_TOKENS
from chat_service.core.telemetry import get_request_id, get_trace_id
...
from chat_service.db.repositories.memory import list_shared_memories, list_skill_memories
from chat_service.services.learning import extract_and_store_learning
from chat_service.clients.llm import LlmServiceError, llm_client
from chat_service.services.llm_metrics import (
    LlmRequestMetrics,
...
async def _chat(
    ...
    *,
    ...
    try:
        completion = await complete_invoice_draft(
            draft,
            idempotency_key=f"chat:{chat_id}",
        )
    ...
    missing_fields = (
        completion.analysis.missing_fields
    ...
    created_invoice = completion.created

    session_state = get_session_state(chat_id)
    session_state["last_document_id"] = created_invoice.document_id
    upsert_session_state(chat_id, session_state)
    clear_document_scope(chat_id)

    response = {
        "status": "created",
        "invoice_id": created_invoice.document_id,
        "invoice_number": created_invoice.invoice_number,
        "subtotal": float(created_invoice.subtotal),
        "total": float(created_invoice.total),
        "currency": created_invoice.currency,
        "pdf_url": created_invoice.download_url,
        "chat_id": chat_id,
    }
    append_chat_message(
        chat_id=chat_id,
        role="assistant",
        content=f"Invoice created — {created_invoice.invoice_number}",
        metadata=response,
    )
    if learn_from_turn:
        await _learn_from_turn(
