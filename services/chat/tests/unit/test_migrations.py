...
def upgrade(database: Path, config: Path | None = None) -> subprocess.CompletedProcess[str]:
    ...
    environment["CHAT_DATABASE_PATH"] = str(database)
...
def test_chat_migration_upgrades_empty_and_existing_database_idempotently(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(database) as connection:
        ...
        assert connection.execute("SELECT version_num FROM alembic_version").fetchone() == ("0001_chat_baseline",)


def test_failed_chat_upgrade_does_not_advance_version(tmp_path: Path) -> None:
    database = tmp_path / "chat.db"
    ...
    (migration_root / "migrations" / "versions" / "0002_fail.py").write_text(
        "from alembic import op\nrevision='0002_fail'\ndown_revision='0001_chat_baseline'\n"
        "branch_labels=None\ndepends_on=None\ndef upgrade():\n    op.execute('CREATE TABLE partial_chat(id INTEGER)')\n    raise RuntimeError('expected')\n"
        "def downgrade():\n    pass\n",
        encoding="utf-8",
    )
